...
* **State Isolation:** Every test gets a clean, fresh browser context via fixtures. This guarantees that if a test changes the language to Persian or corrupts `localStorage` in one run, it wont leak state or break the next test down the line.